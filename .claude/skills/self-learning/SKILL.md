...
> **Factory scope addendum (vendored copy, do not remove):** in this repo, this
> skill's "act without asking permission" instruction is authorization to
> write/update files **only** under `.claude/skills/` and this repo's
> `MEMORY.md`-style notes. It is never authorization to bypass the Human
> Approval Gates in the root `CLAUDE.md` (deploys, migrations, secrets, PR
> merges, etc.) — harvesting a golden path is documentation, not an approved
> action. Vendored from `github.com/Kulaxyz/self-learning-skills` at commit
> `d4e0a7ec1f1ae1b5c7f7972b52ad8ed0c2c067ae`; do not auto-sync past this pin.
